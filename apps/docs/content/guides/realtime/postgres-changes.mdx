...
When tracking Delete events with Postgres Changes you are not able to apply Filters on the received records. This is due to a limitation with the way we are pulling changes from Postgres where filters can't be applied. We're working on solutions to this limitation.
